...
RealT DesignSvmSmoBodyC::GetKernelValue(int I1, int I2)
{
  ...
  register float* elAdr = kernelCache + ((SizeT(I1) * SizeT(I1 + 1)) / 2 + I2);
  register float cacheVal = *elAdr;
...
int DesignSvmSmoBodyC::TakeStep(int I1, int I2)
{
  ...
  if(s < 0.)
  {
    register double gamma = lambda2 - lambda1;
    if(gamma < 0.)
    {
      L = 0.;
      H = _c1 + gamma;
  ...
  {
    int objectID = objectsToUse[k];
    //register double tl = lambdas[objectID];
