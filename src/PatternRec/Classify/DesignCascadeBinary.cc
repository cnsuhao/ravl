...
namespace RavlN {
  ...
  DesignCascadeBinaryBodyC::DesignCascadeBinaryBodyC(const DesignClassifierSupervisedC & design, UIntT maxStages, RealT targetFalseRejection) :
      DesignCascadeBodyC(design),
      m_maxStages(maxStages),
      m_targetFalseRejection(targetFalseRejection)
  {
  }
      ...
      DesignCascadeBodyC(strm)
  {
    ...
    strm >> m_maxStages >> m_targetFalseRejection;
  }
  ...
  bool DesignCascadeBinaryBodyC::Save(std::ostream &out) const
  {
    if (!DesignCascadeBodyC::Save(out))
      return false;
    int version = 0;
    out << ' ' << version << ' ' << m_maxStages << ' ' << m_targetFalseRejection;
    return true;
  }
      ...
      const FeatureSelectorC & featureSelection)
  {
    ...
    RavlAssertMsg(trainingSet.Sample2().LabelSums().Size() == 2,
           "DesignCascadeBinaryBodyC::Apply() only supports two classes.");
