...
namespace RavlN
{
  ...
  public:
    ...
    T &Access(const K &key,const T &default = T());
    //: Access key, if it does exists create a new bin with value 'def'
    //!param: key - Key to lookup.
    //!param: default - Default value to assign to entry if it doesn't exist.
    //!return: Reference to the new entry.

    T &AccessCopy(const K &key,const T &default = T());
    //: Access key, if it does not exist create a new bin with a copy of value 'def'
    //!param: key - Key to lookup.
    //!param: default - Default value to assign a copy of if entry doesn't exist.
    //!return: Reference to the new entry.
