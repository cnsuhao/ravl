...
bool ShotDetBodyC::Run() {
    ...
    //get memory iterator (memory access)
    MemIterC frameIter("frame");
    frameIter.MoveToFirst(); //go to first elem & wait till available
    ...
    for(;!frameIter.Eof(); frameIter.Next()) 
      {
        const MemItemC& item = *frameIter; 
        ...
        if (InShot) {  // look for end of shot
          if (isBoundary) {
            ...
            if ((FrameNo-FirstFrame).getFrameCount() >= (int)MinShotLgth) {
              ...
              Add(MemItemC(FirstFrame, FrameNo-fieldSubsample));
            }
            InShot = false;
          } 
        }
    ...
    if (InShot && ((LastFrame-FirstFrame+1).getFrameCount() >= (int)MinShotLgth))
      {  // if in shot, last frame is end of shot
        ...
        Add(MemItemC(FirstFrame, LastFrame));
      }
    return true;
  }
