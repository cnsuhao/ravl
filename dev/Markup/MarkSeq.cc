...
int MarkSeq(int nargs,char *args[]) 
{
  ...
  bool deinterlace    = option.Boolean("di",  "Deinterlace incoming images. ");
  RealT delay = option.Real("t",deinterlace ? 0.02 : 0.04,"Delay between frames. ");
  bool directDraw     = option.Boolean("dd",  "Direct draw. (For realtime playback of large images) ");
  bool verb           = option.Boolean("v",   "Verbose mode. ");
  bool simpleOnly     = option.Boolean("sc",  "Display Simple Controls only. ");
  ...
  bool noSeek         = option.Boolean("ns",  "Suppress seeking. ");
  bool forceScrollBox = option.Boolean("fs",  "Force scroll box ");
  bool listFormats    = option.Boolean("lf",  "Print list of available data formats. ");
  bool listConversions= option.Boolean("lc",  "Print list of available data converters. ");
  StringC marksFile = option.String ("mf", "","Marks filename");
  StringC classFile = option.String ("cf", "","Classes filename: file contains one class label per line");
  ...
  MarkupElemC markupElem(vidout, verb, classFile);
  markupElem.Load(marksFile);

  ConnectRef(guiPlayControl.SigUpdateFrameNo(), markupElem, &MarkupElemC::CallbackFrameChange, 1);
  ...
  table.GUIAddObject(menuBar,
                     0,4,0,1,(GtkAttachOptions) (GTK_FILL | GTK_EXPAND),(GtkAttachOptions) GTK_FILL);
  table.GUIAddObject(markupElem.GetPrevMarkWidget(),
                     0,1,1,2,(GtkAttachOptions) (GTK_FILL | GTK_EXPAND),GTK_FILL);
  table.GUIAddObject(markupElem.GetFrameNumberWidget(),   
                     1,2,1,2,(GtkAttachOptions) (GTK_FILL | GTK_EXPAND),GTK_FILL);
  table.GUIAddObject(markupElem.GetNextMarkWidget(),
                     2,3,1,2,(GtkAttachOptions) (GTK_FILL | GTK_EXPAND),GTK_FILL);

  ButtonC saveBut = ButtonC("Save marks");
  table.GUIAddObject(saveBut,  
                     3,4,1,2,(GtkAttachOptions) (GTK_FILL | GTK_EXPAND),GTK_FILL);
  ConnectRef(saveBut.Signal("clicked"),markupElem, &MarkupElemC::CallbackSave, marksFile);

  table.GUIAddObject(scrolledVideo,
                     0,4,2,3,(GtkAttachOptions) (GTK_FILL| GTK_EXPAND),(GtkAttachOptions)(GTK_FILL| GTK_EXPAND),5,5);
  table.GUIAddObject(guiPlayControl,
                     0,4,3,4,(GtkAttachOptions) (GTK_FILL | GTK_SHRINK),(GtkAttachOptions)GTK_FILL);
  table.GUIAddObject(grab,
                     2,4,4,5,(GtkAttachOptions) (GTK_FILL| GTK_EXPAND),(GtkAttachOptions)(GTK_FILL));
  table.GUIAddObject(examine,
                     0,2,4,5,(GtkAttachOptions) (GTK_FILL| GTK_EXPAND),(GtkAttachOptions)(GTK_FILL));

  table.GUIAddObject(markupElem.GetMousePosXWidget(),            
                     4,5,0,1,(GtkAttachOptions) (GTK_FILL | GTK_EXPAND),GTK_FILL);
  table.GUIAddObject(markupElem.GetMousePosYWidget(),            
                     4,5,1,2,(GtkAttachOptions) (GTK_FILL | GTK_EXPAND),GTK_FILL);
  table.GUIAddObject(markupElem.GetControlPannel(),
                     4,5,2,5,(GtkAttachOptions) (GTK_FILL | GTK_EXPAND),(GtkAttachOptions)(GTK_FILL));
