...
#include "ExtendedKalmanFilter.hh"
...
int main()
{
  ...
  MotionModelC odometry;
  // Set up the measurement model - for the vision measurement
  MeasurementModelC vision;

  // Set up the EKF with the two models
  ExtendedKalmanFilterC ekf(3); // not that the '3' does anything - ?FIXME?
  ekf.SystemModel( odometry ); // prediction
  ekf.MeasurementModel( vision ); // correction
