...
namespace RavlN {
  ...
  bool FFmpegAudioPacketStreamBodyC::FirstAudioStream(IntT &videoStreamId,IntT &codecId) {
    ...
    for (IntT i = 0; i < pFormatCtx->nb_streams; i++) {
      ...
      frameRateBase = avstream->time_base.den/sampleRate*5945;
...
*/
  ...
  bool FFmpegAudioPacketStreamBodyC::Open(const StringC &filename) {
    ...
    if(pFormatCtx->nb_streams >= 1) {
      positionRefStream = -1;
      frameRate     = avstream->time_base.num;
      frameRateBase = avstream->time_base.den*5945/sampleRate;
  ...
  FFmpegAudioPacketC FFmpegAudioPacketStreamBodyC::Get() {
    ...
    if(packet.StreamIndex() == -1) {
      if(packet.DecodeTimeStamp() != (Int64T) AV_NOPTS_VALUE)
        currentTimeStamp = Time2Frame(packet.DecodeTimeStamp()) + 1;
    }
    return packet;
  }
  ...
  bool FFmpegAudioPacketStreamBodyC::GetAttr(const StringC &attrName,RealT &attrValue) {
    if(attrName == "framerate") {
      ...
      //cerr << "FFmpegAudioPacketStreamBodyC::GetAttr Framerate = num=" << st->time_base.num << " den=" << st->time_base.den << "\n";
