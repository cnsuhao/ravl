...
//! userlevel=Normal
  ...
  bool IplImage2RavlImage(IplImage* src, ImageC<PixelT> & dest)
  {
    ...
    case IPL_DEPTH_8U: case IPL_DEPTH_8S:
      ...
      case 1: 
        status = DPTypeConvert(ImageC<ByteT>(src->height, src->width, (ByteT*)src->imageData, false), dest);
        break;
      case 3:
        status = DPTypeConvert(ImageC<ByteBGRValueC>(src->height, src->width, (ByteBGRValueC*)src->imageData, false), dest);
        break;
      default:
        return false;
      }
    ...
    case IPL_DEPTH_64F:
      switch (src->nChannels) {
      case 1: 
        status = DPTypeConvert(ImageC<RealT>(src->height, src->width, (RealT*)src->imageData, false), dest);
        break;
      case 3:
        status = DPTypeConvert(ImageC<RealBGRValueC>(src->height, src->width, (RealBGRValueC*)src->imageData, false), dest);
        break;
      default:
        return false;
      }
